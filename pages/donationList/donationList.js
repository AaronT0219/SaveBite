function initDonationListPage() {
  console.log("Donation List page initialized.");

  const list = document.getElementById('list');
  const LS_KEY = 'donationItems';
  const LS_REMOVED = 'removedFromManage';
  ...
  if (missing > 0) {
    alert(`There are ${missing} item(s) missing pickup info (pickup location / availability / contact). Please complete them soon.`);
  }
  ...
  list.addEventListener('click', e => {
    ...
    if (act === 'delete') {
      ...
      if (manageId && !removed.includes(manageId)) {
        removed.push(manageId);
        writeLS(LS_REMOVED, removed);
      }
      card.remove();
      return;
    }
  ...
  function render(data) {
    list.innerHTML = '';
    if (!data.length) {
      list.innerHTML = '<p style="color:#666;margin:8px 0;">No donated items yet.</p>';
      return;
    }
    data.forEach(it => {
