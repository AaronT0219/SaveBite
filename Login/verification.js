...
let timeLeft = 60; // Default to 1 minute
...
async function fetchRemainingTime() {
    ...
    try {
        const response = await fetch("getRemainingTime.php", {
            method: "POST",
            headers: {
        ...
        if (data.success) {
            if (data.alreadyActivated) {
                // Account is already activated, show appropriate message
                showAlreadyActivatedStep();
                return;
            }
...
// Function to show already activated step
function showAlreadyActivatedStep() {
    // Hide all other steps
    document.getElementById("step1").classList.add("d-none");
    document.getElementById("step2").classList.add("d-none");
    document.getElementById("successStep").classList.add("d-none");

    // Show already activated step
    document.getElementById("alreadyActivatedStep").classList.remove("d-none");

    // Start redirect countdown for activated account
    startRedirectCountdownActivated();
}
...
document
    ...
    .addEventListener("submit", async (e) => {
        ...
        if (newPassword !== confirmPassword) {
            // Show error on confirm password field instead of general error
            const confirmErrorMessage =
                confirmPasswordInput.parentElement.querySelector(
                    ".error-message"
                );
            confirmPasswordInput.style.borderBottomColor = "#dc3545";
...
// Redirect countdown for already activated page
function startRedirectCountdownActivated() {
    let countdown = 3;
    const timer = document.getElementById("redirectTimerActivated");
...
if (confirmPasswordInput && newPasswordInput) {
    ...
    // Also clear the error when new password changes
